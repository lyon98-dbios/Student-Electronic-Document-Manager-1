...
] = "postgresql://postgres:password@localhost:5432/student_doc"
...
class DocumentForm(FlaskForm):
    file = FileField(
        ...
        validators=[
            ...
            FileAllowed(
                ALLOWED_EXTENSIONS,
                "Only DOCX, TXT, PPTX, JPG, PNG, JPEG, DOC, and PDF files allowed.",
            ),
        ],
    )
...
def save_user(first_name, last_name, matric_no, passkey, department, phone_number):
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                query,
                (first_name, last_name, matric_no, passkey, department, phone_number),
            )
...
def user_index():
    ...
    documents = retrieve_document(doc_id)
...
def generate_otp():
    ...
    if matric_number:
        ...
        return render_template(
            "generate_otp.html", otp=otp, matric_number=matric_number
        )
    else:
...
def edit_user():
    if request.method == "POST":
        ...
        cursor.execute(
            query, (first_name, last_name, department, phone_number, user_id)
        )
        cursor.commit()
        cursor.close()
